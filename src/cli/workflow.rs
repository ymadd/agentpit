...
use crate::exec::{McpConfigGuard, WorkflowManagerExec, is_supported_manager};
...
pub(crate) async fn run_capture(
    ...
    let prompt = if mcp_mode {
        build_manager_prompt_mcp(&goal, &agents, depth, max_depth, max_calls, logger.run_id())
    } else {
    ...
    let manager_exec = WorkflowManagerExec {
        backend: manager,
        child_env: guard::child_env(depth.saturating_add(1), logger.run_id(), &self_path),
...
/// self path, the dispatch grammar, and the instruction to END with a synthesis section.
#[allow(clippy::too_many_arguments)]
...
mod tests {
    ...
    fn self_path_is_shell_quoted_for_paths_with_spaces() {
        ...
        let text =
            build_manager_prompt("goal", &agents, "/Users/a b/bin/agentpit", 1, 3, 8, "run-1");
    ...
    fn mcp_prompt_uses_mcp_tools_and_omits_bash_grammar() {
        ...
        let text = build_manager_prompt_mcp("ship the feature", &agents, 1, 3, 8, "run-9");
    ...
    fn cli_prompt_has_no_mcp_tool_names() {
        ...
        let agents = [BackendId::Gemini];
        let text = build_manager_prompt("goal", &agents, "/bin/agentpit", 1, 3, 8, "run-1");
